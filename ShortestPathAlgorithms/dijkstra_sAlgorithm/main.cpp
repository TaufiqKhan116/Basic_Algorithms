...
N.B. Dijkstra's algorithm uses a data structure for storing and querying partial solutions sorted by distance from the start.
While the original algorithm uses a min-priority queue and runs in time Theta((|V|+|E|)\log |V|)} Theta ((|V|+|E|)\log |V|)}
(where |V| is the number of nodes and |E| is the number of edges), it can also be implemented in Theta(|V|^2) Theta(|V|^2)
using an array -Wikipidea
*/
class Dijkstra
{
    int cost[size][size];
  ...
  private :
    void shortestPaths(int source)
    {
        ...
        //Initializing distances from source to all vertices (If any vertex is unreachable from source the distance is infinity)
        for(int i = 0; i < size; i++)
        {
        ...
        for(int i = 1; i < size; i++)
        {
            ...
            int min = infinity;
            int minVrtx;
            for(int j = 0; j < size; j++) // This is to be replaced by priority queue
            {
